...
?>
	...
	<?php
		echo $this->Form->input('how_many', array(
    		...
    		'empty' => '(choose one)',
    		'label' => 'How many cigarettes per day do you usually smoke?'
		));
		...
		echo $this->Form->input('diff_non_smoking', array(
    		...
    		'empty' => '(choose one)',
    		'label' => 'Do you find it difficult to stop smoking in non-smoking areas?'
		));
		
		echo $this->Form->input('most_hate', array(
    		'options' => array('1'=> 'The first one in the morning','0'=>'Other'),
    		'empty' => '(choose one)',
    		'label' => 'Which cigarette would you most hate to give up?'
		));
	
		echo $this->Form->input('more_morning', array(
    		'options' => array('1'=> 'Yes','0'=>'No'),
    		'empty' => '(choose one)',
    		'label' => 'Do you smoke more frequently in the first hours after waking than during the rest of the day?'
		));
    
		echo $this->Form->input('smok_in_bed', array(
    		'options' => array('1'=> 'Yes','0'=>'No'),
    'empty' => '(choose one)',
    		'label' => 'Do you smoke if you are so ill that you are in bed most of the day?'
		));
